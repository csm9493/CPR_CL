...
def get(seed=0, fixed_order=False, pc_valid=0, tasknum = 50):
    ...
    if not os.path.isdir('data/binary_omniglot/'):
        os.makedirs('data/binary_omniglot')
        
        filename = 'data/Permuted_Omniglot_task50.pt'
...
#                 data[i][s]['x'] = torch.stack(data[i][s]['x']).view(-1, size[0], size[1], size[2])
                ...
                torch.save(data[i][s]['x'],os.path.join(os.path.expanduser('data/binary_omniglot'), 'data' + str(i) + s + 'x.bin'))
                torch.save(data[i][s]['y'],os.path.join(os.path.expanduser('data/binary_omniglot'), 'data' + str(i) + s + 'y.bin'))
        torch.save(ncla_dict, os.path.join(os.path.expanduser('data/binary_omniglot'), 'ncla_dict.pt'))

    else:
        
        ncla_dict = torch.load(os.path.join(os.path.expanduser('data/binary_omniglot'), 'ncla_dict.pt'))
        # Load binary files
#         ids=list(shuffle(np.arange(tasknum),random_state=seed))
        ...
        for i in range(tasknum):
            ...
            for s in ['train', 'test', 'valid']:
                data[i][s] = {'x': [], 'y': []}
                data[i][s]['x'] = torch.load(os.path.join(os.path.expanduser('data/binary_omniglot'), 
                                                          'data' + str(ids[i]) + s + 'x.bin'))
                data[i][s]['y'] = torch.load(os.path.join(os.path.expanduser('data/binary_omniglot'), 
