...
def get(seed=0,pc_valid=0.10, tasknum = 10):
    ...
    for s in ['train','test']:
        data[0][s]={'x':[],'y':[]}
        data[0][s]['x']=torch.load(os.path.join(os.path.expanduser('data/binary_cifar10'),'data'+s+'x.bin'))
        data[0][s]['y']=torch.load(os.path.join(os.path.expanduser('data/binary_cifar10'),'data'+s+'y.bin'))
