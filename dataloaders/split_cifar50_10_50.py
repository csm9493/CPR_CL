...
#     ids=list(range(1,11))
    ...
    data[5] = dict.fromkeys(['name','ncla','train','test'])
    for s in ['train','test']:
        data[5][s]={'x':[],'y':[]}
        data[5][s]['x']=torch.load(os.path.join(os.path.expanduser('data/binary_cifar10'),'data'+s+'x.bin'))
        data[5][s]['y']=torch.load(os.path.join(os.path.expanduser('data/binary_cifar10'),'data'+s+'y.bin'))
    data[5]['ncla']=len(np.unique(data[5]['train']['y'].numpy()))
    data[5]['name']='cifar10'
